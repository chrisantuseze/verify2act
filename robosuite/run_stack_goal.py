...
from goal_renderer import StackGoalRenderer
import numpy as np
import argparse
...
def run_heuristic_policy(env_name: str = "Stack4"):
    ...
    # Goal renderer — must be created after the env so cube objects exist.
    # render_goal() is called after HeuristicStackPolicy.__init__() resets the
    # env (settling objects), ensuring the base cube's position is known.
    goal_renderer = StackGoalRenderer(env, camera="agentview", image_size=512)
    
    # Create policy (calls env.reset() internally — objects settle here).
    policy = HeuristicStackPolicy(env)

    # Render goal image anchored to the settled base-cube position.
    print("Rendering goal image...")
    goal_rgb = goal_renderer.render_goal()
    if goal_rgb is not None:
        goal_path = Path("goal_rendered.png")
