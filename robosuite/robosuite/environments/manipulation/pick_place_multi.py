...
class PickPlaceMulti(ManipulationEnv):
    """
    ...
    def __init__(
        ...
        # Allowed object types (default order)
        _allowed = ["milk", "bread", "cereal", "can", "bottle", "lemon", "squarenut"]

        # If user specifies object_types, validate and use them; otherwise use all allowed
        if object_types is None:
            self.available_objects = _allowed.copy()
        else:
            # normalize to lowercase
            self.available_objects = [o.lower() for o in object_types]
            for o in self.available_objects:
                assert o in _allowed, f"invalid object type '{o}' - choose from {_allowed}"

        # Map object name -> id (0..N-1) for the chosen objects
    ...
    def reward(self, action=None):
        ...
        if self.reward_scale is not None:
            ...
            if self.single_object_mode == 0:
                # normalize by number of active objects (avoid division by zero)
                n_objs = max(1, len(self.objects))
                reward /= float(n_objs)
        return reward
    ...
    def _get_placement_initializer(self):
        ...
        # Define edge margin to keep objects away from bin edges
        edge_margin = 0.08  # Increased margin to keep objects away from edges
        
        # Calculate safe sampling bounds within the bin
        bin_x_half = self.model.mujoco_arena.table_full_size[0] / 2 - edge_margin
        bin_y_half = self.model.mujoco_arena.table_full_size[1] / 2 - edge_margin
    ...
    def _construct_visual_objects(self):
        ...
        """
        # Dynamically construct visual objects for the chosen object types
        visual_class_map = {
            "milk": MilkVisualObject,
            "bread": BreadVisualObject,
            "cereal": CerealVisualObject,
            "can": CanVisualObject,
            "bottle": CanVisualObject,
            "lemon": BreadVisualObject,
    ...
    def _construct_objects(self):
        ...
        Function that can be overriden by subclasses to load different objects.
        """
        # Dynamically construct collision objects for the chosen object types
        obj_class_map = {
            "milk": MilkObject,
            "bread": BreadObject,
            "cereal": CerealObject,
...
class PickPlaceMulti3(PickPlaceMulti):
    """
    Multi-object version using three objects: milk, bread, cereal.
    """
...
class PickPlaceMulti4(PickPlaceMulti):
    """
    Multi-object version using four objects: milk, bread, cereal, can (default).
    """
