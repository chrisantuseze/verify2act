...
def parse_args():
    ...
    parser.add_argument("--negative-prompt", type=str, default=None, help="Negative prompt for inference CFG")
    ...
    parser.add_argument("--image-guidance-scale", type=float, default=2.5)
        ...
        for i, line in enumerate(handle):
            if i == index:
                ...
                if "action_params" in row and "cartesian_target" in row["action_params"]:
                    ...
                    prompt += f" at loc {ct[0]:.2f} {ct[1]:.2f} {ct[2]:.2f}"
...
def extract_wm_version(adapter_dir: str) -> str:
    """Extract WM version from adapter dir path.
    
    Example: 'verify2act/output/wm_v12/best/unet_lora' -> 'wm_v12'
    """
    if not adapter_dir:
        return "default"
    parts = Path(adapter_dir).parts
    for part in parts:
        if part.startswith("wm_"):
            return part
    return "unknown"
...
def main():
    ...
    # Extract WM version from adapter dir and create structured output directory
    wm_version = extract_wm_version(args.adapter_dir)
    run_dir_name = f"{wm_version}_{args.transition_index}"
    output_base = Path(args.output_path).parent / run_dir_name
    output_base.mkdir(parents=True, exist_ok=True)

    # Save individual images
    input_image_path = output_base / "input.png"
    gt_image_path = output_base / "ground_truth.png"
    generated_image_path = output_base / "generated.png"
    collage_path = output_base / "collage.png"
    meta_path = output_base / "run_meta.json"

    image_in.resize(original_size).save(input_image_path)
    ...
    if gt_path is not None and Path(gt_path).exists():
        ...
        print(f"Saved ground truth:    {gt_image_path}")
    else:
        gt_image = None

    image_out.save(generated_image_path)
    ...
    meta = {
        ...
        "gt_image": str(gt_image_path) if gt_path is not None else None,
        "prompt": prompt,
